...
export default function UploadPage() {
  ...
  const onUpload = async () => {
    ...
    try {
      // Perform the actual backend upload
      const result = await uploadPdf(file, userId, userName);
      
      // Store IDs for the Chat session
      localStorage.setItem("coeus_job_id", result.data.job_id);
      localStorage.setItem("coeus_pdf_name", result.data.filename);

      // Success - Move to chat
      navigate("/chat");
    } catch (err) {
      setError(err.message || "Failed to upload. Check your server connection.");
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fa] text-[#1e293b] px-6 py-10 overflow-hidden relative">
      ...
      >
        <motion.div
          className="rounded-[24px] border border-[#e2e8f0] bg-white p-8 shadow-sm relative overflow-hidden"
        >
          <Sparkles />
          ...
          <p className="mt-3 text-[#64748b]">Choose a PDF file and we’ll start chatting with it.</p>
          ...
          >
            ...
            <input
              type="file" accept="application/pdf" disabled={uploading}
            ...
            <motion.button
              onClick={onUpload} disabled={!file || uploading}
              className={`mt-6 w-full rounded-[12px] px-5 py-3 font-semibold transition-all ${
                file && !uploading ? "bg-blue-600 text-white hover:shadow-lg" : "bg-[#e2e8f0] text-[#64748b] cursor-not-allowed"
              }`}
            >
              {uploading ? "Analyzing PDF..." : "Start Chatting →"}
            </motion.button>
            ...
            <button onClick={() => navigate("/")} className="mt-3 w-full rounded-[12px] px-5 py-3 font-semibold text-[#64748b] hover:bg-[#f1f5f9]">
              ← Back
            </button>
          </motion.div>
        </motion.div>
